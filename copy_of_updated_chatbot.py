# -*- coding: utf-8 -*-
"""Copy of UPDATED CHATBOT

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1sJ3rbrSjlearVOGq6oRBMcgZ7J3JgTOF
"""
...
# Cleaning of each text conversation
def clean_text(text):
    ...
    text = re.sub(r"can't", "can not", text, flags=re.I)
    text = re.sub(r"that's", "that is", text, flags=re.I)
    text = re.sub(r"there's", "there is", text, flags=re.I)
    text = re.sub(r"what's", "what is", text, flags=re.I)
...
# Getting each answer/question
pairs = []
for conversation in conversations_ids:
    for i in range(0, len(conversations_ids) - 1, 2):
        try:
            pairs.append([clean_text(id2line[conversation[i]]), clean_text(id2line[conversation[i+1]])])
        except IndexError:
            break
...
datafile = os.path.join("formatted_movie_lines_2.txt")
...
def readVocs(datafile, corpus):
    print("Reading lines...")
    # Read the file and split into lines
    lines = open(datafile, encoding='utf-8').\
        read().strip().split('\n')
    # Split every line into pairs and normalize
    pairs = [[getString(s) for s in l.split('\t')] for l in lines]
    voc = Voc(corpus)
    return voc, pairs
...
def trainIters(model_name, voc, pairs, encoder, decoder, encoder_optimizer, decoder_optimizer, embedding, encoder_n_layers, decoder_n_layers, save_dir, n_iteration, batch_size, print_every, save_every, clip, corpus, loadFilename):
    start = time.time()
...
def save_model(encoder, decoder, encoder_optimizer, decoder_optimizer, checkpoint_dir):
    ...
    }, os.path.join(checkpoint_dir, 'trained_models.pth'))

# Assuming you have encoder, decoder, encoder_optimizer, and decoder_optimizer defined
checkpoint_dir = './checkpoints'  # Directory to save the models
save_model(encoder, decoder, encoder_optimizer, decoder_optimizer, checkpoint_dir)
...
# Assuming you have encoder, decoder, encoder_optimizer, and decoder_optimizer defined
checkpoint_path = './checkpoints/trained_models.pth'
load_model(checkpoint_path)

# Function to generate response using the trained models
...
<body>
    ...
    <script>
        function sendMessage() {
            ...
            xhr.send(JSON.stringify({ input_sentence: input }));
